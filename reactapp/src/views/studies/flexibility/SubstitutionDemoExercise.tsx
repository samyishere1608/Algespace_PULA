...
import { CompletedDemo, IsolatedIn } from "@/types/flexibility/enums.ts";
...
import { SubstitutionDemo } from "@components/flexibility/substitution/demo/SubstitutionDemo.tsx";
import { Variable } from "@/types/flexibility/variable.ts";
import { Coefficient } from "@/types/math/coefficient.ts";
import { ErrorBoundary } from "react-error-boundary";

export function SubstitutionDemoExercise(): ReactElement {
        ...
        <div className={"full-page"} style={{ background: "linear-gradient(180deg, var(--blue-background) 0%, #044a6d 100%)", paddingBottom: "1rem" }}>
            ...
            <div className={"flexibility-view__container"}>
                <div className={"flexibility-view__contents"}>
                    ...
                    <p><Trans ns={TranslationNamespaces.Flexibility} i18nKey={FlexibilityTranslations.SUBSTITUTION_DEMO_1} /></p>
                    <p>{t(FlexibilityTranslations.SUBSTITUTION_DEMO_2)}</p>
                    <SubstitutionDemo system={[firstEquation, secondEquation]} firstVariable={firstVariable} isolatedVariables={[IsolatedIn.None, IsolatedIn.First]} />
                    <button className={"button primary-button return-study-view-button"} onClick={() => {
                        setFlexibilityStudyDemosCompleted(studyId, CompletedDemo.Substitution);
...
function getFirstEquation(): FlexibilityEquation {
    const leftTerms: FlexibilityTerm[] = [
        new FlexibilityTerm(math.fraction(3), "x"),
        new FlexibilityTerm(math.fraction(-2), "y")
    ];
    const rightTerms: FlexibilityTerm[] = [new FlexibilityTerm(math.fraction(1), null)];
    return new FlexibilityEquation(leftTerms, rightTerms);
}

function getSecondEquation(): FlexibilityEquation {
    const leftTerms: FlexibilityTerm[] = [new FlexibilityTerm(math.fraction(1), "x")];
    const rightTerms: FlexibilityTerm[] = [
        new FlexibilityTerm(math.fraction(3), "y"),
        new FlexibilityTerm(math.fraction(-2), null)
    ];
    return new FlexibilityEquation(leftTerms, rightTerms);
}
